...
public class ModSlider extends GuiButton {
    ...
    public ModSlider(int idIn, int x, int y, int width, int height, String name, float min, float max, float step, float defaultValue, int decimals) {
        this(new GuiResponder() {
            @Override
            public void setEntryValue(int arg0, boolean arg1) {}

            @Override
            public void setEntryValue(int arg0, float arg1) {}

            @Override
            public void setEntryValue(int arg0, String arg1) {}
    ...
    public void func_175218_a(float value, boolean updateDisplay) {
        ...
        if (updateDisplay) {
            this.responder.setEntryValue(this.id, this.getValue());
        }
    }
    ...
    protected void mouseDragged(Minecraft mc, int mouseX, int mouseY) {
        if (this.visible) {
            ...
            this.drawTexturedModalRect(this.x + (int)(this.valueToPosition(this.getValue()) * (float)(this.width - 8)), this.y, 0, 66, 4, 20);
            this.drawTexturedModalRect(this.x + (int)(this.valueToPosition(this.getValue()) * (float)(this.width - 8)) + 4, this.y, 196, 66, 4, 20);
        }
    }
