...
public class GuiConfig extends ModScreen {
	...
	public void addTopContainer(int x, int y) {
		...
		int showTextStringWidth = mc.fontRenderer.getStringWidth(showTextString);
		int updateManagerStringWidth = Math.max(20, mc.fontRenderer.getStringWidth(updateManagerString) + 8);
	...
	public void addVisibilityButtons(int x, int y) {
		...
		textField = new GuiTextField(GuiButtons.F_TEXT.id, fontRenderer, x, GuiButtons.F_TEXT.getY(y), 148, 20);
	...
	public void addColorButtons(int x, int y) {
        ...
        int stringWidth = fontRenderer.getStringWidth(labelString);
		marginBackgroundLabel = new GuiLabel(fontRenderer, GuiButtons.L_MARGIN_BACKGROUND.id, x+5, GuiButtons.L_MARGIN_BACKGROUND.getY(y), stringWidth, 20, 0xffffff);
        marginBackgroundLabel.addLine(labelString);

		marginBackgroundField = new ModTextField(GuiButtons.L_MARGIN_BACKGROUND.id, fontRenderer, x+5 + stringWidth + 10, GuiButtons.L_MARGIN_BACKGROUND.getY(y), 148 - 5 - stringWidth - 10, 20);
